...
struct DocumentCameraView: UIViewControllerRepresentable {
    ...
    @Binding var scanText: AttributedString
...
//                          lineBreak.draw(at: CGPoint(x: 0, y: y), withAttributes: headingAttributes)
        ...
        func documentCameraViewController(_ controller: VNDocumentCameraViewController, didFinishWith scan: VNDocumentCameraScan) {
                ...
                for observation in observations {
                    ...
                    if oldBoundingBoxes.count > 0 {
                        ...
                        /*
                            ...
                            TODO: May need to average results (talk with da team)
                         */
                        if (oldBoundingBoxes[oldBoundingBoxes.count - 1].y - observation.topLeft.y >= 0.04) || (observation.topLeft.x - oldBoundingBoxes[oldBoundingBoxes.count - 1].x >= 0.04) {
                            print("Creating new paragraph")
                            // Create a new paragraph
                            paragraphs.append(currentParagraph)
                            currentParagraph.removeAll()
                            oldBoundingBoxes.removeAll()
                        }
                    }
                    
                    currentParagraph.append(bestCandidate.string)
                    
                    oldBoundingBoxes.append(observation.topLeft)
                }
            }
            ...
            let pdfDocument = PDFDocument() // Contains complete PDF's data
            ...
            parent.pdfDocument = PDFDocument(data: CreatePDFData(paragraphs))
